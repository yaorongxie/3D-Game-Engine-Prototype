...
bool Mesh::Load(const WCHAR* fileName, AssetManager* pAssetManager)
{
	...
	mVertexBuffer = new VertexBuffer(Graphics::Get(), vertices.data(), numVerts, vertSize,
...
Mesh::Mesh(AssetManager* pAssetManager, const VertexBuffer* pVertexBuffer, Material* pMaterial)
{
...
{
	// Set the material of this object to be active 
	// (1) Set the corresponding shader to active + (2) Set the corresponding textures to active
	...
	{
		PROFILE_SCOPE(VextexBufferDraw);
		// Draw the vertices
		mVertexBuffer->Draw();
	}
}
